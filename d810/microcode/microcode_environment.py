...
class MicroCodeEnvironment(object):
    ...
    def define(self, mop: mblock_t, value: int) -> int:
        if mop.t == mop_r:
            ...
            return value
        raise EmulationException("Defining an unsupported mop type '{0}': '{1}'".format(mop_type_to_string(mop.t), format_mop_t(mop)))

    def _lookup_mop(self, searched_mop: mop_t, mop_value_dict: Dict[mop_t, int], new_mop_value: Union[None, int] = None, auto_define=True, raise_exception=True) -> int:
    ...
    def assign(self, mop: mop_t, value: int, auto_define=True) -> int:
        ...
        raise EmulationException("Assigning an unsupported mop type '{0}': '{1}'".format(mop_type_to_string(mop.t), format_mop_t(mop)))
