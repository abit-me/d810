...
class FlowOptimizationRule(OptimizationRule):
    ...
    def configure(self, kwargs):
        ...
        if "whitelisted_functions" in self.config.keys():
            ...
            optimizer_logger.info("Whitelisted functions for {0}: {1} -> {2}".format(self.__class__.__name__, self.whitelisted_function_ea_list, func_name_list))
        if "blacklisted_functions" in self.config.keys():
            ...
            optimizer_logger.info("Blacklisted functions for {0}: {1} -> {2}".format(self.__class__.__name__, self.blacklisted_function_ea_list, func_name_list))
